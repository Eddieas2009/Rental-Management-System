...
$stmt = $pdo->prepare("SELECT  CASE MONTH(expense_date)
    WHEN 1 THEN 'Jan'
    WHEN 2 THEN 'Feb'
    WHEN 3 THEN 'Mar'
    WHEN 4 THEN 'Apr'
    ...
    WHEN 12 THEN 'Dec'
    ELSE 'Invalid Month'
END AS month_name, sum(amount)totalamount from `expenses` where YEAR(expense_date)=YEAR(curdate()) GROUP BY month_name");
$stmt->execute();
